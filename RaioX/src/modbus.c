...
int fill_buffer(uint8_t *tx_buffer, mb_package pkg) {
    ...
    *ptr++ = pkg.code;
    ...
    // Adiciona os dados da requisição, como: endereço inicial e qtd regs
    if (pkg.offset > 0 && pkg.data != NULL) {
        memcpy(ptr, pkg.data, pkg.offset);
        ptr += pkg.offset;
    }
    ...
    short crc = get_crc(tx_buffer, ptr - tx_buffer);
    memcpy(ptr, &crc, sizeof(short));
    ptr += sizeof(short);

    return ptr - tx_buffer;
}

void print_buffer(const uint8_t *buffer, int len) {
    printf("Mensagem final (%d bytes): ", len);
    for (int i = 0; i < len; i++) {
